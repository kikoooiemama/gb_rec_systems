...
class MainRecommender:
    ...
    def _get_recommendations_als(self, user, model, N=5):
        """Рекомендации через стандартные библиотеки implicit"""
        ...
        filter_items = [] if self.fake_id is None else [self.itemid_to_id[self.fake_id]]
        res = model.recommend(userid=self.userid_to_id[user],
                              user_items=csr_matrix(self.user_item_matrix_for_pred.loc[user]).tocsr(),
                              N=N,
                              filter_already_liked_items=False,
                              filter_items=filter_items,
                              recalculate_user=True)
    ...
    def get_als_recommendations(self, user, n=5):
        ...
        self._update_dict(user_id=user)
        return self._get_recommendations_als(user, model=self.model, N=n)
